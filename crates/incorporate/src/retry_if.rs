...
/// will be returned.
pub async fn with_timeout<F, Value>(duration: Duration, future: F) -> Result<Value, DoprfErrorNew>
where
    F: Future<Output = Result<Value, DoprfErrorNew>>,
{
    let mut future = Box::pin(future).fuse();
    let mut delay = futures_timer::Delay::new(duration).fuse();
    futures::select_biased! {
        res = future => res,
        _ = delay => Err(DoprfErrorNew::Timeout { after: duration })
    }
}
